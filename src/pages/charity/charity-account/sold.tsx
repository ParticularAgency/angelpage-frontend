...
const SoldItemsPage = () => {
  ...
  const handleSortChange = (sortBy: string) => {
    ...
    if (sortBy === 'Awaiting shipping') {
      ...
      sortedItems = sortedItems.filter(item => item.stages.shipped && !item.stages.delivered);
                ...
                <div className="flex items-center gap-3 md:gap-[6px] justify-between w-full"> 
                  <div className="sold-item-info-box flex items-center gap-[33px] md:gap-4">
                    <div className="sold-item-img">
                     <Image src={item.productImageSrc} className="h-[65px] object-cover" alt="sold product item image" width={70} height={65} />
                  </div>
      ...
      <div className="col-span-8 md:col-span-7 pt-[23px] pr-[45px] md:pr-4 md:pl-4 pb-[50px] bg-[#F1F1F7] pl-5 sm:col-span-full sold-product-states-item-cont">
        {selectedItem && (
          <> 
            ...
            <div className="mt-1">
              <p><span className="body-bold-small"></span> {selectedItem.saleDate}</p>
            </div>
            ...
            <div className="relative mt-[15px]">
              <div className="flex flex-col space-y-6 sm:space-y-3 pl-7 relative sold-product-items-main-item-content">
                <div className="flex p-4 bg-mono-0 relative flex-col gap-1 items-start">
                  ...
                  <p className="purchase-confirmed-date-time forms text-mono-80">1 Nov 2024</p>
                </div>
                ...
                <div className="flex p-4 bg-mono-0 relative flex-col gap-1 items-start">
                  ...
                  <div className="flex items-start flex-col gap-2 justify-start w-full">
                    ...
                    {!selectedItem.stages.shipped ? (
                      ...
                      <p className="body-bold-small mt-1 flex items-center gap-1">Tracking: <span className="text-primary-color-100"> DV38474905867352</span></p>
                      </>
                    )}
                  </div>
                </div>
                ...
                {/* Tracking Error */}
                {trackingError && <p className="body-bold-small text-error">{trackingError}</p>}
  ...
  className={`w-6 h-6 flex justify-center items-center rounded-full absolute dots-item-indicator left-[-38px] bottom-0 ${
    selectedItem.stages.proceedsSent ? 'bg-[#6A0398]' : 'states-not-complete !left-[-36px]'
  }`}
>
  {selectedItem.stages.proceedsSent && (
    <Image src="/images/icons/checkmark.svg" alt="checkmark icon" width={16} height={16} />
                  ...
                  <p className="body-bold-small">Sale proceedings sent to <span className="sold-product-price-sent-to-selectedcharity">Samaritans Ltd</span></p>
                  <p className="forms text-mono-70 mt-1">7 Nov 2024</p>
                         {selectedItem.stages.proceedsSent && (
              <div className="">
                {(() => {
                  const { saleProceeds } = calculateFees(selectedItem.productPrice);
                  return (
                    <>
                      <p className="body-bold-small text-mono-100 sold-product-90-percent-need-to-go-selected-charity">{saleProceeds}</p>
                    </>
                  );
                })()}
              </div>
