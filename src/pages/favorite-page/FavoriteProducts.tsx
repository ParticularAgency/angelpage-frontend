...
const FavoriteProductListing = () => {
    const [products] = useState<Product[]>(productData); // Ensure productData matches Product type
    ...
    const filterAndSortProducts = (filters: Filters, sort: SortOptions, query: string) => {
        ...
        if (filters.productBrand.length > 0) {
            updatedProducts = updatedProducts.filter((product) =>
                product.productBrand && filters.productBrand.includes(product.productBrand)
            );
        }
        if (filters.productCondition.length > 0) {
            updatedProducts = updatedProducts.filter((product) =>
                product.productCondition && filters.productCondition.includes(product.productCondition)
            );
        }
    ...
    const filteredProducts = useMemo(() => {
        const updatedProducts = filterAndSortProducts(filters, sort, searchQuery);
        return updatedProducts;
