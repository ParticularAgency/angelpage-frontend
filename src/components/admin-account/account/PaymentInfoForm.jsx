...
const PaymentInfoForm = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingIndex !== null) {
        ...
        if (paymentId) {
          await axios.put(
            `${process.env.NEXT_PUBLIC_API_URL}/admin/profile/payments/${paymentId}`,
            payment,
            {
              headers: {
                Authorization: `Bearer ${session?.token}`,
        ...
        const response = await axios.post(
          `${process.env.NEXT_PUBLIC_API_URL}/admin/profile/payments`,
          payment,
          {
            headers: {
              Authorization: `Bearer ${session?.token}`,
  ...
  const handleDelete = async () => {
    if (paymentToDelete) {
      try {
        await axios.delete(
          `${process.env.NEXT_PUBLIC_API_URL}/admin/profile/payments/${paymentToDelete}`,
          {
            headers: {
              Authorization: `Bearer ${session?.token}`,
            },
