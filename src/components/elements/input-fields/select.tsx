...
const Select: React.FC<SelectProps> = ({
    ...
    return (
        <div className={`select-wrapper flex flex-col gap-2 ${className}`}>
            {label && <label htmlFor={id || name} className="font-secondary text-body-form font-normal leading-[150%] text-mono-100">{label}</label>}
            <select
                name={name}
                id={id}
                required={required}
                ...
                value={value}
                onChange={onChange}
                className={`select-field h-10 py-[11px] px-[8px] bg-transparent text-body-caption font-normal leading-[150%] font-secondary text-mono-60 focus:text-mono-100 ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
                style={{
